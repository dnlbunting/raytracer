...
class BiConvex(CompositeObject):
    ...
    def __init__(self, centre, height, r1, r2, R, thickness, ref_index):
        ...
        self.R1 = normalise(isVector(R))
        self.radius2 = np.abs(r2)
        self.R2 = -1.*normalise(isVector(R))
        ...
        self.bfd = lambda wl : self.f(wl) * (1 - (self.ref_index(wl)-1)*self.d/(self.ref_index(wl)*np.abs(self.radius2)) )
